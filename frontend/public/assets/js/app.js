...
function setActiveNav(name){
  elAll('.nav-btn').forEach(b=>{ b.classList.toggle('active', b.dataset.page === name); });
}
document.addEventListener('click', e=>{
  const btn = e.target.closest('.nav-btn');
  if(btn){ const p = btn.dataset.page; history.pushState({p},'',`#${p}`); setActiveNav(p); loadPage(p); }
});
...
async function pollStatus(){
  try{ const r = await fetch('/api/status'); const j = await r.json(); const s = el('#srv-status'); if(s) s.textContent = j.server?.online ? 'online' : 'offline'; }catch(e){}
}
